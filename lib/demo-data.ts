...
export async function resetDemoDataForOrg(
  db: AdminClient,
  orgId: string
): Promise<DemoResetResult> {
  const { data: existingSignals, error: signalsLookupError } = await db
    .from("signals")
    .select("id")
    .eq("org_id", orgId);
  throwIfError("signals lookup", signalsLookupError);
  ...
  if (signalIds.length > 0) {
    ...
    throwIfError("channel_actions", error);
  }

  for (const table of ["audit_trail", "data_issues", "signals", "accounts"] as const) {
    const { error } = await db.from(table).delete().eq("org_id", orgId);
    throwIfError(table, error);
  }

  const { error: accountError } = await db
    .from("accounts")
    .insert(DEMO_ACCOUNTS.map((account) => ({ ...account, org_id: orgId })));
  throwIfError("accounts", accountError);

  const { error: signalError } = await db
    .from("signals")
    .insert(DEMO_SIGNALS.map((signal) => ({ ...signal, org_id: orgId })));
  throwIfError("signals", signalError);
  ...
  for (const [name, playbook] of Object.entries(PLAYBOOKS)) {
    ...
    throwIfError(`playbook ${name}`, error);
  }

  const { data: accounts, error: accountLookupError } = await db
    .from("accounts")
    .select("id,name")
    .eq("org_id", orgId);
  throwIfError("account lookup", accountLookupError);

  const accountByName = new Map((accounts ?? []).map((account) => [account.name, account.id]));
  const issueRows = DATA_ISSUES.filter((issue) => accountByName.has(issue.account)).map(
    (issue) => ({
      account_id: accountByName.get(issue.account)!,
      org_id: orgId,
  ...
  if (issueRows.length > 0) {
    const { error } = await db.from("data_issues").insert(issueRows);
    throwIfError("data_issues", error);
  }
