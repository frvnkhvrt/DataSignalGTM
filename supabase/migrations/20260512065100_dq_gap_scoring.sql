-- DQ Gap Scoring: severity impact tracking and transactional gap RPCs.
-- Existing open gaps are treated as already reflected in account DQ scores;
-- this migration backfills impact metadata but does not retroactively decrement.

ALTER TABLE public.data_issues
  ADD COLUMN IF NOT EXISTS score_impact integer NOT NULL DEFAULT 0;

ALTER TABLE public.data_issues
  DROP CONSTRAINT IF EXISTS data_issues_score_impact_check;

ALTER TABLE public.data_issues
  ADD CONSTRAINT data_issues_score_impact_check
  CHECK (score_impact >= 0);

UPDATE public.data_issues
SET score_impact = CASE severity
  WHEN 'high' THEN 8
  WHEN 'medium' THEN 5
  WHEN 'low' THEN 2
  ELSE 0
