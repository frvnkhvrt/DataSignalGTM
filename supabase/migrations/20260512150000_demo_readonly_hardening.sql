-- Defense in depth: demo users can read the shared workspace, but cannot
-- mutate GTM data directly through RLS-aware clients or mutable RPCs.
...
DROP POLICY IF EXISTS "members can delete signals" ON public.signals;
...
BEGIN
  IF public.is_demo_user() THEN
    RAISE EXCEPTION 'Demo mode is read-only.'
      USING ERRCODE = '42501';
  END IF;
