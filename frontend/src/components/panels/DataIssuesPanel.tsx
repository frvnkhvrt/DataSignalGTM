...
/** Read-only review of account fields that feed DQ / trust. No score mutation. */
    ...
    <Sheet open={!!account} onOpenChange={(o) => !o && onClose()}>
      ...
      >
        <SheetHeader className="px-6 py-4 border-b border-zinc-800">
          ...
          <SheetTitle className="text-zinc-100 text-base">{account?.name ?? ""}</SheetTitle>
        </SheetHeader>

        <div className="px-6 py-5 border-b border-zinc-800">
          <div className="text-xs uppercase tracking-wide text-zinc-500">DQ score</div>
          <div className="mt-2 flex items-baseline gap-2">
            <span className={`text-4xl font-semibold font-mono tabular-nums ${scoreColor}`}>
          ...
          <p className="mt-3 text-xs text-zinc-500">
            Playbook opens at DQ ≥ 90. Held outreach band is typically DQ 75–89. Update source records,
            then refresh this page after data sync.
          </p>
        </div>

        <div className="px-6 py-4 space-y-2">
          <div className="text-xs uppercase tracking-wide text-zinc-500 mb-2">Fields</div>
          {rows.map((row) => (
            <div
              key={row.label}
              className={`rounded-md border px-3 py-2.5 text-sm ${
                row.present ? "border-zinc-800 bg-zinc-900" : "border-amber-500/35 bg-amber-500/5"
              }`}
            >
              ...
              <div className="mt-1 text-xs text-zinc-400 break-all">{row.detail}</div>
            </div>
          ))}
        </div>
      </SheetContent>
